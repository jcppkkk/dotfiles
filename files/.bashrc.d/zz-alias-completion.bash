...
function alias_completion {
    ...
    while read -r line; do
        ...
        # shellcheck disable=SC2154 # alias_tokens is referenced but not assigned
        ...
        read -r -a alias_arg_words <<<"$alias_args"
        ...
        if [[ ! " ${completions[*]} " =~ $alias_cmd ]]; then
            if [[ -n "$completion_loader" ]]; then
                ...
                [[ $? -eq 124 ]] || continue
                mapfile -t split_cmd <<<"$alias_cmd"
                completions+=("${split_cmd[@]}")
            else
                continue
            fi
        fi
